...
async function guardarMiembro() {
  ...
  const { error } = await db.from('equipo').insert({
    nombre,
    cargo: document.getElementById('eq-cargo').value,
    telefono: document.getElementById('eq-tel').value.trim() || null,
    especialidad: document.getElementById('eq-esp').value.trim() || null
  });
...
// Fix createMiembro to handle RLS error gracefully
